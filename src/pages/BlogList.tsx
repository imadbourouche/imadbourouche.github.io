        ...
        <div className="min-h-screen pt-32 pb-20 px-6 bg-slate-50 dark:bg-slate-900 transition-colors">
            <div className="max-w-3xl mx-auto space-y-12">
                ...
                {loading ? (
                    ...
                    <div className="grid gap-6">
                        {posts.map((post) => (
                            <Link
                                key={post.slug}
                                to={`/blog/${post.slug}`}
                                className="group block bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-teal-500 dark:hover:border-teal-500 hover:shadow-lg transition-all"
                            >
                                <div className="space-y-3">
                                    ...
                                    <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-2 transition-colors">
                                        {post.summary}
                                    </p>

                                    <div className="flex flex-wrap gap-2 pt-1">
                                        {post.tags.map(tag => (
                                            <span key={tag} className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs rounded-full font-medium transition-colors">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                    ...
                    <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 transition-colors">
                        <div className="space-y-6">
                            <div className="w-16 h-16 bg-teal-50 dark:bg-teal-900/30 rounded-full flex items-center justify-center mx-auto text-teal-600 dark:text-teal-400 transition-colors">
